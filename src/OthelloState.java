...
public class OthelloState {
			...
			for (int j = 0; j < boardSize; j++) {
				if (board[i][j] == NOTHING) {
					...
					for (int k = 0; k < offs_x.length && !moveFound; k++) {
						...
						while (current_x + offs_x[k] >= 0
								&& current_x + offs_x[k] < boardSize
								&& current_y + offs_y[k] >= 0
								&& current_y + offs_y[k] < boardSize
	...
	public void applyMove(OthelloMove move) {
		...
		for (int i = 0; i < offs_x.length; i++) {
			...
			while (current_x + offs_x[i] >= 0
					&& current_x + offs_x[i] < boardSize
					&& current_y + offs_y[i] >= 0
					&& current_y + offs_y[i] < boardSize
